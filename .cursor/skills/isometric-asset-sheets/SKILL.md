...
description: Generate sprite sheet images for FloodGuard Surabaya using the GenerateImage tool. Use when creating new game assets, sprite sheets, vehicle sprites, building sprites, or any visual assets for the isometric flood-management simulation. Ensures consistent format, sizing, and isometric projections.
---

# Isometric Asset Sheet Generation

Generate consistent sprite sheets for FloodGuard Surabaya using the GenerateImage tool.
...
```
Using the same format as [reference if available] - red background, 6 rows, 5 columns - I need you to make a new asset sheet with [ASSET TYPE] for FloodGuard Surabaya. 
...
```
Using the same format as this image - red background, 6 rows, 5 columns - I need you to make a new asset sheet with AIRPLANES for FloodGuard Surabaya. 
...
```
Red background, 6 rows, 5 columns - asset sheet with VEHICLES for FloodGuard Surabaya.
...
```
Red background, 6 rows, 5 columns - asset sheet with BOATS for FloodGuard Surabaya.
